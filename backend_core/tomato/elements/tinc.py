...
class TincVPN(ConnectingElement, Element):
	...
	TYPE = TypeName.TINC_VPN
	HOST_TYPE = None
	DIRECT_ACTIONS = False
	DIRECT_ACTIONS_EXCLUDE = []
	CAP_CHILDREN = {TypeName.TINC_ENDPOINT: [ST_CREATED, ST_PREPARED]}
	...
	ACTIONS.update({
		...
		ActionName.START: StatefulAction(action_start, check=Element.checkTopologyTimeout, allowedStates=[ST_PREPARED], stateChange=ST_STARTED),
...
class TincEndpoint(ConnectingElement, Element):
	...
	TYPE = TypeName.TINC_ENDPOINT
	HOST_TYPE = TypeName.TINC
	CAP_CHILDREN = {}
