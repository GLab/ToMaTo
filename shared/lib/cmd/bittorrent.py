...
def startClient(path, bwlimit=10000, minport=8010, maxport=8020, checkHashes=True):
	global _clientPid, _clientConfig
	if _clientPid:
		return
	assert os.path.exists(path)
	_clientConfig = {"path": path, "bwlimit": bwlimit} 
	_clientPid = spawn(["btlaunchmany", ".", "--max_upload_rate", str(bwlimit), "--minport", str(minport), "--maxport", str(maxport), "--check_hash", str(1 if checkHashes else 0)], cwd=path, daemon=False)
	try:
