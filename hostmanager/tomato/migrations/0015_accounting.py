...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for el in orm.Element.objects.all():
            if not el.usageStatistics:
                el.usageStatistics = orm.UsageStatistics.objects.create(attrs="{}")
                el.save()
        for con in orm.Connection.objects.all():
            if not con.usageStatistics:
                con.usageStatistics = orm.UsageStatistics.objects.create(attrs="{}")
                con.save()
