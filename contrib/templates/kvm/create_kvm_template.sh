...
get DISTRO "Distribution (nickname)" jessie
IFS="/" read -ra DISTRO_PARTS <<< "$DISTRO"
DISTRO=${DISTRO_PARTS[0]}
VARIANT=${DISTRO_PARTS[1]}
case "$DISTRO" in
  squeeze|wheezy|jessie|lucid|natty|oneiric|precise|quantal|raring|saucy|trusty|utopic|vivid|wily|xenial|yakkety)
    if [ "$VARIANT" == "" ]; then
      PRESEED="/$DISTRO.preseed.txt"
      DEST="$DISTRO-$ARCH.qcow2"
    else
      PRESEED="/${DISTRO}_${VARIANT}.preseed.txt"
      DEST="$DISTRO-$VARIANT-$ARCH.qcow2"
    fi
...
KVM="kvm -usbdevice tablet -m 512 -smp sockets=1,cores=1 -nodefaults -vga vmware -k de -enable-kvm -net nic,model=e1000 -net user"
if [ "$1" == "--silent" ]; then
