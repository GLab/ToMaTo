...
get DISTRO "Distribution (squeeze,precise,oneiric,natty,lucid)"
case "$DISTRO" in
  squeeze)
    TYPE="debootstrap"
  ...
  lucid|natty|oneiric|precise)
    ...
    if ! [ -f /usr/share/debootstrap/scripts/$DISTRO ]; then
      cp /usr/share/debootstrap/scripts/maverick /usr/share/debootstrap/scripts/$DISTRO \
      || cp /usr/share/debootstrap/scripts/hardy /usr/share/debootstrap/scripts/$DISTRO
    fi
    ;;
  fedora-17)
    TYPE="febootstrap"
...
case $TYPE in
  debootstrap)
    debootstrap --arch $ARCH --components "$COMPONENTS" $DISTRO "$TARGET" "$MIRROR" 2>&1 | indent
    ;;
  febootstrap)
    febootstrap $DISTRO "$TARGET" "$MIRROR" 2>&1 | indent
    ;;
esac

echo
echo "Configuring VM container..."
vzctl set $VMID --applyconfig basic --save
...
(vzctl exec2 $VMID /run.sh 2>&1; R=$?) | indent
