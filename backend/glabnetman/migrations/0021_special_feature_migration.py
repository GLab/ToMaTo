...
class Migration(SchemaMigration):
	...
	def forwards(self, orm):
		for host in orm.Host.objects.all():
			sf = orm.SpecialFeature(host=host, feature_type="internet", feature_group=host.group.name, bridge=host.public_bridge)
			sf.save()
		for con in orm.InternetConnector.objects.all():
			spec = orm.SpecialFeatureConnector(name=con.name, resources=con.resources, type="special", pos=con.pos, state=con.state, topology=con.topology, feature_type="internet", feature_group="")
			spec.save()
			for c in con.connection_set.all():
				c.connector = spec
				c.save()
			con.delete()
	
	def backwards(self, orm):		
		raise Exception("no backward migration implemented")
