...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		for sf in orm.SpecialFeature.objects.all():
			sfg = orm.SpecialFeatureGroup.objects.filter(feature_type=sf.feature_type, group_name=sf.feature_group_name)
			if len(sfg) >= 1:
				sfg = sfg[0]
			else:
				sfg = orm.SpecialFeatureGroup.objects.create(feature_type=sf.feature_type, group_name=sf.feature_group_name)
			sf.feature_group = sfg
			sf.save()					
	
	def backwards(self, orm):
		for sf in orm.SpecialFeature.objects.all():
			sfg = sf.feature_group
			sf.feature_group_name = sfg.group_name
			sf.feature_type = sfg.feature_type
			sf.save()	
	...
	models = {
		...
		'tomato.specialfeature': {
			...
			'feature_group': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['tomato.SpecialFeatureGroup']", 'null': 'True'}),
