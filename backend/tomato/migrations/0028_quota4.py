...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		for user in orm["tomato.user"].objects.all():
			if user.quota:
				continue
			quota = orm["tomato.quota"]()
			quota.used = orm["tomato.usage"].objects.create()
			quota.monthly = orm["tomato.usage"].objects.create(cputime=10000000.0, memory=2500000000.0, diskspace=25000000000.0, traffic=25000000000.0)
			quota.used_time = time.time()
			quota.continous_factor = 1.0
			quota.save()
			user.quota = quota
			user.save()
	...
	models = {
		...
		'tomato.user': {
			...
			'attrs': ('tomato.lib.db.JSONField', [], {}),
			'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
			'last_login': ('django.db.models.fields.FloatField', [], {'default': '1397466224.281776'}),
