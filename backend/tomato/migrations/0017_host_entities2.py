...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		for cls in ["tomato.external_network_endpoint", "tomato.tinc_endpoint", "tomato.udp_endpoint",
					"tomato.kvmqm", "tomato.kvmqm_interface",  
					"tomato.openvz", "tomato.openvz_interface",
					"tomato.repy", "tomato.repy_interface"]:
			for obj in orm[cls].objects.all():
				child = obj.element
				if not child:
					continue 
				assert not child.topology_element
				child.topology_element = obj.element_ptr
				child.save()
		for obj in orm["tomato.connection"].objects.all():
			child = obj.connection1
			if child:
				assert not child.topology_element
				assert not child.topology_connection
				child.topology_connection = obj
				child.save()
			child = obj.connection2
			if child:
				assert not child.topology_element
				assert not child.topology_connection
				child.topology_connection = obj
				child.save()
			child = obj.connectionElement1
			if child:
				assert not child.topology_element
				assert not child.topology_connection
				child.topology_connection = obj
				child.save()
			child = obj.connectionElement2
			if child:
				assert not child.topology_element
				assert not child.topology_connection
				child.topology_connection = obj
	...
	models = {
		...
		'tomato.user': {
			...
			'attrs': ('tomato.lib.db.JSONField', [], {}),
			'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
			'last_login': ('django.db.models.fields.FloatField', [], {'default': '1389176727.413372'}),
