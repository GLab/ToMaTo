...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		for obj in orm["tomato.user"].objects.filter(totalUsage=None):
			obj.totalUsage = orm["tomato.usagestatistics"].objects.create()
			obj.save()
		for obj in orm["tomato.organization"].objects.filter(totalUsage=None):
			obj.totalUsage = orm["tomato.usagestatistics"].objects.create()
			obj.save()
		for obj in orm["tomato.host"].objects.filter(totalUsage=None):
			obj.totalUsage = orm["tomato.usagestatistics"].objects.create()
			obj.save()
	...
	models = {
		...
		'tomato.user': {
			...
			'attrs': ('tomato.lib.db.JSONField', [], {}),
			'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
			'last_login': ('django.db.models.fields.FloatField', [], {'default': '1396859498.38925'}),
