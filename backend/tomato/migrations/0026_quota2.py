...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		keep = []
		for obj in orm["tomato.topology"].objects.all():
			keep.append(obj.totalUsage.pk) 
		for obj in orm["tomato.user"].objects.all():
			keep.append(obj.totalUsage.pk) 
		for obj in orm["tomato.element"].objects.all():
			keep.append(obj.totalUsage.pk) 
		for obj in orm["tomato.connection"].objects.all():
			keep.append(obj.totalUsage.pk) 
		for obj in orm["tomato.host"].objects.all():
			keep.append(obj.totalUsage.pk) 
		for obj in orm["tomato.organization"].objects.all():
			keep.append(obj.totalUsage.pk) 
		for obj in orm["tomato.hostelement"].objects.all():
			keep.append(obj.usageStatistics.pk) 			
		for obj in orm["tomato.hostconnection"].objects.all():
			keep.append(obj.usageStatistics.pk) 			
		orm["tomato.usagestatistics"].objects.exclude(pk__in=keep).delete()
		for ur in orm["tomato.usagerecord"].objects.all():
			ur.usage = orm["tomato.usage"].objects.create(cputime=ur.cputime, memory=ur.memory, diskspace=ur.diskspace, traffic=ur.traffic)
			ur.save() 
	...
	models = {
		...
		'tomato.user': {
			...
			'attrs': ('tomato.lib.db.JSONField', [], {}),
			'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
			'last_login': ('django.db.models.fields.FloatField', [], {'default': '1397208113.366702'}),
